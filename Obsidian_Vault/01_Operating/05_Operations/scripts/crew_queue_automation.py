...
# Trello credentials — loaded from vault credential file, never hardcoded (OPSEC)
import sys
sys.path.insert(0, str(Path(__file__).parent))
try:
    from credential_loader import load_trello_credentials
    TRELLO_KEY, TRELLO_TOKEN = load_trello_credentials()
except Exception:
    # Fallback: parse from Operating Paperwork markdown
    _cred = Path(r"D:\Work\Torus Coffee Company LLC\Obsidian_Vault\01_Operating\Operating Paperwork\Trello_API_Credentials.md")
    _txt = _cred.read_text(encoding="utf-8") if _cred.exists() else ""
    def _grab(label):
        m = re.search(rf"{label}\s*\n?`?([A-Za-z0-9]+)`?", _txt)
        return m.group(1) if m else ""
    import re as _re
    re = _re
    TRELLO_KEY = _grab("API Key")
    TRELLO_TOKEN = _grab("Token")
if not TRELLO_KEY or not TRELLO_TOKEN:
    raise RuntimeError("Trello credentials not found — check Trello_API_Credentials.md")
    ...
    for qname, qid in torus_queues.items():
        ...
        for card in cards:
            ...
            if action == "sync":
                ...
                print("  ⏭️  Noop: already synced, no outbox message (anti-spam)")
            else:
                results["failed"] += 1
                results["failed_details"].append({
                    "card_id": cid,
