# ⚓ PIRATE CREW CLI TOOL
## Command-line Interface for Fleet Operations

**Purpose:** Crew can query, control, and monitor infrastructure from terminal  
**Language:** Python 3.11+  
**Dependencies:** docker, requests, click, tabulate, rich  
**Usage:** Available to all crew on all 3 ships (via Tailscale)
...
```bash
# On any ship (SQUIDSTATION, PINKCADY, STEALTHATTACK)
pip install click requests tabulate rich pyyaml

# Clone or copy pirate_crew_cli.py to /opt/pirate-crew/
mkdir -p /opt/pirate-crew
# Copy file here
chmod +x /opt/pirate-crew/pirate_crew_cli.py

# Add to PATH (optional)
export PATH=$PATH:/opt/pirate-crew
```
...
```bash
# Check fleet status (all 3 ships)
pirate-crew status

# Check specific ship
pirate-crew status --ship squidstation

# List all containers across fleet
pirate-crew containers --all

# View container logs
pirate-crew logs torus-website --tail 50

# Check health of specific service
pirate-crew health torus-pos

# Execute command in container
pirate-crew exec torus-inventory "SELECT * FROM inventory LIMIT 5"

# View metrics (CPU, memory)
pirate-crew stats torus-website

# Restart container
pirate-crew restart torus-pos

# Deploy update
pirate-crew deploy torus-website v1.0.5

# Check GPU status (STEALTHATTACK)
pirate-crew gpu-status

# Submit AI job
pirate-crew job-submit inference_v2 pytorch/pytorch:2.0 inference.py

# Check backup status
pirate-crew backup status

# Trigger manual backup
pirate-crew backup now

# View alerts
pirate-crew alerts --last 24h

# Get configuration
pirate-crew config get torus-website

# Set configuration
pirate-crew config set torus-website REDIS_HOST 100.106.235.103
```
...
```bash
#!/bin/bash
# install-pirate-cli.sh

echo "📦 Installing Pirate Crew CLI..."
...
# Create symlink
sudo ln -sf /opt/pirate-crew/pirate_crew_cli.py /usr/local/bin/pirate-crew

# Verify
pirate-crew status

echo "✓ Pirate Crew CLI installed"
echo "Usage: pirate-crew --help"
```
...
stats           → Show CPU/memory stats
restart         → Restart a container
health          → Check service health
exec            → Execute command
gpu-status      → Check GPU metrics
job-submit      → Submit AI job
alerts          → View alerts
...
This CLI tool runs on all crew machines, can query all 3 ships, and doesn't touch Miss Pink's infrastructure build.

Crew can now command the fleet from terminal. 🏴‍☠️
